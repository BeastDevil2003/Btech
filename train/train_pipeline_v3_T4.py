...
# Saves 4 publication-quality plots after training completes:
#   1. Loss curves     (train + val loss vs epoch)
#   2. AUC curves      (train + val AUC vs epoch)
#   3. Accuracy curves (train overall + real acc + fake acc + val acc)
#   4. Learning rate   (backbone + rest LR vs epoch)
# ===========================================================================

def plot_training_history(history: dict, save_dir: str, batch_size: int = 2):
    """
    Generate and save training graphs from history dict.
    Called automatically at end of training.
    """
    os.makedirs(save_dir, exist_ok=True)

    epochs = history.get("epoch", list(range(1, len(history.get("train_loss", [])) + 1)))
    # Filter out NaN for cleaner plots
    def clean(lst):
        return [v if (v is not None and not (isinstance(v, float) and math.isnan(v))) else float("nan")
                for v in lst]
    ...
    val_loss_clean = [v for v in clean(history.get("val_loss", [])) if not math.isnan(v)]
    if val_loss_clean:
        best_idx = val_loss_clean.index(min(val_loss_clean))
        ax1.axvline(x=epochs[best_idx], color="red", linestyle=":", alpha=0.5,
    ...
    ax2.set_title("AUC vs Epoch", fontweight="bold")
    ax2.set_xlabel("Epoch"); ax2.set_ylabel("AUC")
    ax2.set_ylim(bottom=max(0.0, min(
        min([v for v in clean(history.get("train_auc", [0])) if not math.isnan(v)] or [0]) - 0.05,
        0.4)))
    ax2.legend(); ax2.grid(alpha=0.3)
    # Annotate best val AUC
    val_auc_clean = [v for v in clean(history.get("val_auc", [])) if not math.isnan(v)]
    if val_auc_clean:
        best_auc_val = max(val_auc_clean)
        best_auc_idx = val_auc_clean.index(best_auc_val)
        ax2.annotate(f"Best: {best_auc_val:.4f}",
                     xy=(epochs[best_auc_idx], best_auc_val),
                     xytext=(epochs[best_auc_idx], best_auc_val - 0.04),
                     fontsize=9, ha="center", color="#DC2626",
                     arrowprops=dict(arrowstyle="->", color="#DC2626", lw=1.2))

    # ── Plot 3: Accuracy breakdown ────────────────────────────────────────
    ...
    ax3.set_title("Accuracy vs Epoch", fontweight="bold")
    ...
    # ── Additional: Train vs Val AUC gap (overfitting detector) ──────────
    if history.get("train_auc") and history.get("val_auc"):
        fig2, ax = plt.subplots(figsize=(10, 5))
        train_auc = clean(history["train_auc"])
        val_auc   = clean(history["val_auc"])
        gap       = [t - v if not (math.isnan(t) or math.isnan(v)) else float("nan")
                     for t, v in zip(train_auc, val_auc)]
        ax.bar(epochs, gap, color=["#DC2626" if g > 0.05 else "#16A34A" for g in
               [v if not math.isnan(v) else 0 for v in gap]], alpha=0.7)
    ...
    print(f"  Saved: training_history.png, overfit_monitor.png → {save_dir}/")
...
class FocalLoss(nn.Module):
    """
    Binary Focal Loss.
    alpha=0.5 (balanced classes via WeightedRandomSampler)
    gamma=2.0 (standard for deepfake detection)
    """
...
def train(
    ...
    weight_decay: float = 5e-4,    # 5e-4: balanced between overfit(1e-4) and underfit(1e-3)
    num_workers: int = 0,
    patience: int = 7,
    warmup_epochs: int = 2,
    ...
    for epoch in range(resume_epoch, epochs):
        ...
        # Log — track everything needed for graphs
    ...
    try:
        ...
        print(f"Training graphs saved → {graph_dir}/")
