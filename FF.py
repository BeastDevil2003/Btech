...
def urlopen_with_retries(url, max_retries=5, timeout=30):
    attempts = 0
    while attempts < max_retries:
        try:
            ...
            if attempts >= max_retries:
                raise
            tqdm.write('URL open failed ({}/{}): {}. Retrying in {}s...'.format(
                attempts, max_retries, exc, 5 * attempts))
...
def download_file(url, out_file, report_progress=False, max_retries=5):
    ...
    if not os.path.isfile(out_file):
        ...
        while attempts < max_retries:
            try:
                ...
                if attempts >= max_retries:
                    break
                tqdm.write('Download failed ({}/{}): {}. Retrying in {}s...'.format(
                    attempts, max_retries, exc, 5 * attempts))
                time.sleep(5 * attempts)
        os.remove(out_file_tmp)
        raise Exception('Failed to download {} after {} retries'.format(
            url, max_retries))
    else:
...
def main(args):
    ...
    for dataset in c_datasets:
        ...
        if 'DeepFakeDetection' in dataset_path or 'actors' in dataset_path:
            filepaths = json.loads(urlopen_with_retries(args.base_url + '/' +
            ...
            file_pairs = json.loads(urlopen_with_retries(args.base_url + '/' +
